...
static void button_pressed(const struct device *dev, struct gpio_callback *cb,
		    uint32_t pins)
{
	printk("Button %p / %u pressed at %" PRIu32 "\n", 	dev, pins, k_cycle_get_32());
};
...
const ErrorCode Button::init() {
	...
	gpio_init_callback(&m_pin_cb_data, button_pressed, BIT(m_pin.pin));
	gpio_add_callback(m_pin.port, &m_pin_cb_data);
